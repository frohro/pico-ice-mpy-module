...
static mp_obj_t ice_module_fpga_make_new(const mp_obj_type_t *type, size_t n_args, size_t n_kw, const mp_obj_t *all_args) {
	...
	float min_freq = 48.0f / 65536.0f;
	if (self->frequency <= min_freq || self->frequency > 48.0f) {
		mp_raise_msg_varg(&mp_type_ValueError, MP_ERROR_TEXT("Frequency must be between %.6f and 48.0 MHz"), min_freq);
	}
